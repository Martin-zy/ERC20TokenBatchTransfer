...
var txHashQ = new Queue(function (input, cb) {
    ...
    if (receipt == null) {
        ...
        let timeout = txSender.track(input.timestamp, input.gasPriceHex);
        if (timeout) {
            ...
            txQ.push({
                ...
                amount: input.amount,
                gasLimitHex: "0x18939",
                gasPriceHex: txSender.estimateGasPrice(),
        ...
        txEvent.txSuccess({hash: hash, from: input.from, to: input.to, amount: txSender.hexToInt(input.amount)});
        ...
        if (input.history.length < 4) {
            ...
            txEvent.txFail({hash: hash, from: input.from, to: input.to, amount: txSender.hexToInt(input.amount)});
        }
        cb(null, "fail")
    }
...
var txQ = new Queue(async function (input, cb) {
    ...
    if (hash == null || hash == "tx_send_err" || hash == "invalidAddr") {
