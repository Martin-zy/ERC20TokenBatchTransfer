...
var txHashQ = new Queue(function (input, cb) {
    ...
    if (receipt == null) {
        ...
        input.history.push(hash);
        txEvent.emit('tx_success',{hash:hash,from:input.from,to:input.to,amount:txSender.hexToInt(input.amount)});
        ...
        if (input.history.length < 4) {
            ...
            txEvent.emit('tx_fail',{hash:hash,from:input.from,to:input.to,amount:txSender.hexToInt(input.amount)});
        }
        cb(null, "fail")
    }
